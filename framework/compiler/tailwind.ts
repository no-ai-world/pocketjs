...
export function generateStylesModule(
  c: CompiledStyles,
  fontLineHeights: Record<number, number> = {},
): string {
  const L: string[] = [];
  ...
  L.push("export const FONT_SLOTS: Record<number, { px: number; bold: boolean; lineHeight?: number }> = {");
  for (const slot of c.usedFontSlots) {
    const { px, bold } = fontSlotInfo(slot);
    const lineHeight = fontLineHeights[slot];
    const lineHeightField = lineHeight === undefined ? "" : `, lineHeight: ${lineHeight}`;
    L.push(`  ${slot}: { px: ${px}, bold: ${bold}${lineHeightField} },`);
  }
  L.push("};");
  L.push("");
