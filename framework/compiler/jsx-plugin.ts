...
const HOST_INPUT_PATH = fileURLToPath(new URL("../src/host-input.ts", import.meta.url));
...
const TEXT_EDIT_PATH = fileURLToPath(new URL("../src/text-edit.ts", import.meta.url));
...
> = {
  solid: {
    ...
    subpaths: {
      ...
      "host-input": HOST_INPUT_PATH,
      launcher: LAUNCHER_PATH,
      lifecycle: LIFECYCLE_PATH,
      "text-edit": TEXT_EDIT_PATH,
        ...
        enter(program) {
          program.traverse({
            ...
            JSXText(path) {
              ...
              const normalizedRaw = typeof raw === "string" ? raw.replace(/\r\n?/g, "\n") : raw;
              const normalizedValue = path.node.value.replace(/\r\n?/g, "\n");
              if (typeof normalizedRaw === "string" && normalizedRaw !== normalizedValue) {
