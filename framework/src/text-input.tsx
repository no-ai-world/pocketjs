...
export function TextInput(props: TextInputProps): SolidJSX.Element {
  ...
  const [contentNode, setContentNode] = createSignal<NodeMirror | null>(null);
  ...
  const handlePointer = (x: number, y: number, down: boolean, shift: boolean) => {
    ...
    const n = node();
    if (!n) return;
    // App-supplied toLocal wins; otherwise map into the content node's
    // border box so outer padding is excluded from caret placement.
    const ops = getOps();
    const coordinateNode = contentNode() ?? n;
    const rawLocal = p.toLocal
      ? p.toLocal(x, y)
      : ops.nodeLocalPoint?.(coordinateNode.id, x, y);
    const local =
      typeof rawLocal === "number"
        ? rawLocal === 0
          ? undefined
          : { x: ops.nodeLocalX?.() ?? Number.NaN, y: ops.nodeLocalY?.() ?? Number.NaN }
        : rawLocal ?? undefined;
    if (local && (!Number.isFinite(local.x) || !Number.isFinite(local.y))) {
      throw new Error(`TextInput: host nodeLocalPoint returned non-finite coordinates (${local.x}, ${local.y})`);
    }
    if (down && !prevDown) {
      focusNode(n);
      setFocused(true);
  ...
  createEffect(() => {
    ...
    const handler: EditableHandler = {
      ...
      caretRect: () => {
        ...
        return { node: content.id, x, y: row * p.lineHeight, h: p.lineHeight };
      },
    };
